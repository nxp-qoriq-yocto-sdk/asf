...
/******************************************************************************
 * File Name : ipsecfp.c
 * Description: Contains the routines for ipsec fast path at the device driver level
 * Version  : 0.1
 * Author : Subha
 * Date : October 2009
 ******************************************************************************/
...
static inline void asfIPSecCopyWords(unsigned int *dst, unsigned int *src, int len);
		...
		unsigned int ulSPDContainerIndex)
{
	...
	if ((pSA->ulPkts[0] + pSA->ulPkts[1]) % ulL2BlobRefreshPktCnt_g == 0) {
		TunAddress.IP_Version = 4;
		TunAddress.dstIP.bIPv4OrIPv6  = 0;
		TunAddress.srcIP.bIPv4OrIPv6  = 0;
		TunAddress.dstIP.ipv4addr = pSA->SAParams.tunnelInfo.addr.iphv4.daddr;
		TunAddress.srcIP.ipv4addr = pSA->SAParams.tunnelInfo.addr.iphv4.saddr;
		ASFIPSecCbFn.pFnRefreshL2Blob(ulVSGId, pSA->ulTunnelId, ulSPDContainerIndex,
						ptrIArray_getMagicNum(&(secfp_OutDB),  ulSPDContainerIndex), &TunAddress,
						pSA->SAParams.ulSPI, pSA->SAParams.ucProtocol);
	}
...
int secfp_try_fastPathOutv4 (
	...
	if (pSA) {
		...
		if ((iph->tot_len  > (pSA->ulPathMTU)) || (skb_shinfo(skb1)->frag_list)) {
		...
		if ((iph->tot_len  > (pSA->ulPathMTU)) || (skb_shinfo(skb1)->frag_list)) {
			...
			if (unlikely((iph->tot_len) < pSA->ulPathMTU)) {
				...
				if (pSA->SAParams.bRedSideFragment) {
					ASFIPSEC_DEBUG("Red side fragmentation is enabled");
						if (unlikely(asfIpv4Fragment(skb1,
									 pSA->ulPathMTU + ETH_HLEN + pSA->ulSecOverHead,
									 ETH_HLEN, TRUE, skb1->dev, &skb))) {
						ASF_IPSEC_PPS_ATOMIC_INC(IPSec4GblPPStats_g.IPSec4GblPPStat[ASF_IPSEC_PP_GBL_CNT22]);
						ASF_IPSEC_INC_POL_PPSTATS_CNT(pSA, ASF_IPSEC_PP_POL_CNT22);
						return 1;
						...
						if (!(skb_shinfo(skb1)->frag_list)) {
							...
							(pSA->SAParams.ucCipherAlgo != SECFP_ESP_NULL)) { /* Can fit in 2 fragments */
								...
								if (skb1->prev) {
									...
									skb1->prev->len = (iph->tot_len + (pSA->ulSecOverHead + ucPadLen) - pSA->ulPathMTU);
		...
		ASFIPSEC_DEBUG("outV4: bScatterGather = %d",
			bScatterGatherList);
		ASFIPSEC_DEBUG("outv4: skb = 0x%x skb1 =0x%x",
				(unsigned int) skb, (unsigned int) skb1);
		for (pNextSkb = skb->next; skb != NULL; skb = pNextSkb) {
			skb->next = NULL;
			...
			*(unsigned int *)  &(skb->cb[SECFP_SAD_SAI_MAGIC_INDEX]) =  pSecInfo->outSAInfo.ulSAMagicNumber;
			...
			switch (secout_sg_flag) {
			case SECFP_OUT | SECFP_NO_SCATTER_GATHER:
				if (desc)
					secfp_prepareOutDescriptor(skb,  pSA,
							desc, 0);
			break;
			case SECFP_OUT | SECFP_SCATTER_GATHER:
				if (desc)
					secfp_prepareOutDescriptor(skb,  pSA,
							desc, 0);
			break;
			...
			if (secfp_talitos_submit(pdev, desc,
				(secout_sg_flag & SECFP_SCATTER_GATHER)
				? secfp_outComplete : secfp_outComplete
				, (void *)skb) == -EAGAIN) {
			...
			if (pSA->option[1] != SECFP_NONE) {
				...
				case SECFP_OUT | SECFP_SCATTER_GATHER:
				if (desc)
					secfp_prepareOutDescriptorWithFrags
						(skb, pSA, desc, 1);
				break;
				}
				if (secfp_talitos_submit(pdev, desc,
			...
			ASFIPSEC_PRINT("nf_frags after finish function = %d",
				nr_frags);
#endif
			skb->cb[SECFP_REF_INDEX]--;
			if (skb->cb[SECFP_REF_INDEX] == 0) {
			...
			ASFIPSecCbFn.pFnNoOutSA(ulVSGId , NULL, Buffer, ASFSkbFree, skb1, bSPDContainerPresent, bRevalidate);
			return 1;
		}
		rcu_read_unlock();
		return 0;
		...
		if (!skb->cb[SECFP_OUTB_FRAG_REQD]) {
			...
			if (ptrIArray_getMagicNum(&secFP_OutSATable, *(unsigned int *)&(skb->cb[SECFP_SAD_SAI_INDEX]))
				...
				if (pSA) {
					...
					skb->data += pSA->ulL2BlobLen;
					skb->len -= pSA->ulL2BlobLen;
					asfIpv4Fragment(skb, pSA->odev->mtu, pSA->ulL2BlobLen,
	...
	if (!pSA->SAParams.bUseExtendedSequenceNumber) {
		if (ulSeqNum <= pSA->ulLastSeqNum) {
			...
			if (uiDiff >= pSA->SAParams.AntiReplayWin) {
				...
				snprintf(aMsg, ASF_MAX_MESG_LEN - 1, "Anti-replay window check failed");
				AsfLogInfo.aMsg = aMsg;
				asfFillLogInfo(&AsfLogInfo, pSA);
				ASFIPSEC_WARN("Anti-replay window check failed #1");
				/* Update SA Statistics */
				skb->cb[SECFP_ACTION_INDEX] = SECFP_DROP; /* DROP = 1*/
				return;
			}
			   ...
			   ASF_uint32_t  ulCommonInterfaceId)
{
	...
	if (pSA) {
		...
		*(unsigned int *)&(pHeadSkb->cb[SECFP_HASH_VALUE_INDEX]) = ulHashVal;
		...
		desc = secfp_desc_alloc();
		switch (secin_sg_flag) {
		case SECFP_IN | SECFP_NO_SCATTER_GATHER:
			if (desc)
				secfp_prepareInDescriptor(pHeadSkb,
						pSA, desc, 0);
		break;
		case SECFP_IN | SECFP_SCATTER_GATHER:
			if (desc)
		...
		if (pSA->option[1] != SECFP_NONE) {
			...
			case SECFP_IN | SECFP_SCATTER_GATHER:
				if (desc)
					secfp_prepareInDescriptorWithFrags(
						pHeadSkb, pSA, desc, 1);
			break;
			}
