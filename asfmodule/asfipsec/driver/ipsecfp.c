	...
	if (pSA->SAParams.ucCipherAlgo != SECFP_ESP_NULL) {
		...
		*/
		if (skb_shinfo(skb1)->nr_frags)
			for (ii = 0, jj = 0;
				ii < usPadLen; ii += 4, jj++)
			*(unsigned int *)&(charp[frag->size + ii]) = pad_words[jj];
		else
			for (ii = 0, jj = 0;
				ii < usPadLen; ii += 4, jj++)
			*(unsigned int *) &(pTailSkb->data[pTailSkb->len+ii]) = pad_words[jj];
	} else {
		usPadLen = 0;
	}
	...
	*/
	...
	iph->tos = tos;
		...
		*pOuterIpHdr = (unsigned int *)
		...
		/* Total length = Outer IP hdr + Sec hdr len (inclusive of IV) + payload len + padding length + Trailer len */
		ipv6h->payload_len = ASF_HTONS(orig_pktlen + pSA->usNatHdrSize +
			(unsigned short)pSA->ulSecHdrLen +
			(unsigned short)pSA->ulSecLenIncrease - SECFP_IPV6_HDR_LEN
			+ usPadLen + SECFP_ESP_TRAILER_LEN);
		ASFIPSEC_DBGL2("New payload len %d",
				ASF_NTOHS(ipv6h->payload_len));
...
#if (ASF_FEATURE_OPTION > ASF_MINIMUM)
send_l2blob:
	if (likely(ASFIPSecCbFn.pFnRefreshL2Blob)) {
				...
				ASFFFPIpsecInfo_t *pSecInfo)
{
	...
	ASFIPSEC_FPRINT("Pkt received skb->len = %d, ipv6h->payload_len = %d",
		skb1->len, ASF_NTOHS(ipv6h->payload_len));
			...
			*(unsigned int *)&(skb->cb[SECFP_SAD_SAI_INDEX]))
			...
			if (likely(pSA && pSA->bl2blob)) {
				...
				for (; pOutSkb != NULL; pOutSkb = pTempSkb) {
					...
					pIPSecPPGlobalStats->ulTotOutProcPkts++;
#ifdef CONFIG_DPA
					if (pOutSkb->cb[BUF_INDOMAIN_INDEX]) {
						if (pOutSkb->dev) {
							priv = netdev_priv(pOutSkb->dev);
							dpa_bp = priv->dpa_bp;
							PER_CPU_BP_COUNT(dpa_bp)--;
						}
					}
			...
			ASF_uint32_t ulCommonInterfaceId)
{
	...
	if (pSA) {
		...
		if (skb_shinfo(skb1)->frag_list ||
			...
			if (likely((ASF_NTOHS(ipv6h->payload_len) -
				(pSA->ulSecHdrLen)) < pSA->ulRcvMTU)
			...
			len = ASF_NTOHS(ipv6h->payload_len) + SECFP_IPV6_HDR_LEN;
#else
...
So all these special boundary cases need to be handled for nr_frags*/
		...
		if ((ASF_NTOHS(ipv6h->payload_len) +
				SECFP_IPV6_HDR_LEN) < pSA->validIpPktLen) {
