	...
	*/
	...
	if ((iph->protocol == IPPROTO_ESP) || (iph->protocol == IPPROTO_AH)) {
		if (pFFPIPSecIn) {
			...
			XGSTATS_INC(NonTcpUdpPkts);
			asf_debug("Non IP traffic. EthType = 0x%x", ASF_NTOHS(usEthType));
			goto ret_pkt;
		}
#if (ASF_FEATURE_OPTION > ASF_MINIMUM)
				...
				)
{
	...
	asf_print(" Pkt (%x) skb->len = %d, iph->tot_len = %d",
		pIpsecOpaque, skb->len, host_ip_tot_len);
	...
	if (unlikely((iph->frag_off) & ASF_MF_OFFSET_FLAG_NET_ORDER)) {
		...
		asf_debug("DeFrag & Pull done .. proceed!! skb->len %d iph->tot_len %d fragCnt %d\n",
			  skb->len, host_ip_tot_len, fragCnt);
	...
	if (flow && (iph->ttl > 1)) {
		...
		if (vsgInfo) {
			...
			if ((!flow->bIPsecOut) &&
				...
				if ((flow->odev) && !(flow->odev->flags &
							IFF_UP)) {
					L2blobRefresh =
						ASF_L2BLOB_REFRESH_DROP_PKT;
					goto gen_indications;
				}
		...
		if (iph->protocol == IPPROTO_UDP) {
			XGSTATS_INC(UdpPkts);
			if (((host_ip_tot_len - iphlen) < 8) ||
				(ASF_NTOHS(*(q + 2)) > (host_ip_tot_len - iphlen))) {
			...
			*/
			if (((host_ip_tot_len - iphlen) < trhlen)) {
			...
			tcp_data_len = host_ip_tot_len-iphlen-trhlen;
			asf_debug_l2("TCP_STATE_PROC: tcp_data_len = %d\n", tcp_data_len);
		...
		if (flow->bIPsecOut) {
			...
			if (((skb->len + flow->l2blob_len) >
				...
				if (!asfIpv4Fragment(skb, flow->pmtu,
					...
					for (; pSkb != NULL; pSkb = pTempSkb) {
						...
						if (flow->bPPPoE) {
							/* PPPoE packet.. Set Payload length in PPPoE header */
							*((short *)&(pSkb->data[flow->l2blob_len-4])) = ASF_HTONS(host_ip_tot_len + 2);
						}
			...
			if (flow->bPPPoE) {
				...
				*((short *)&(skb->data[(flow->l2blob_len - tunnel_hdr_len)-4])) =
				ASF_HTONS(host_ip_tot_len + tunnel_hdr_len + 2);
			}
#endif  /* (ASF_FEATURE_OPTION > ASF_MINIMUM) */
...
/*TBD - This code is not used anywhere.
	...
	if (skb->mac_len != ETH_HLEN) {
		...
		if (usEthType == __constant_htons(ETH_P_PPP_SES)) {
			...
			/* PPPoE packet.. Set Payload length in PPPoE header */
			asf_debug_l2("   Adjust PPPOE len (old %u) to %u\n", *((short *)  &(skb->data[x_hh_len-4])),  ASF_HTONS(host_ip_tot_len + 2));
