			...
			ASFBuffer_t abuf)
{
	...
	if (unlikely(nexthdr == NEXTHDR_HOP)) {
		...
		if (ASF_NTOHS(*(unsigned int *)(&pL4hdr[4])) > ip6h->payload_len)
			goto drop_pkt;
		...
		do {
			...
			if (flow->bPPPoE) {
				...
				*((short *)&(skb->data[(flow->l2blob_len -
				tunnel_hdr_len)-4])) =
					ASF_HTONS(skb->len + tunnel_hdr_len + 2);
			}
				...
				)
{
	...
	if (unlikely(nexthdr == NEXTHDR_HOP)) {
		...
		pkt_len = ASF_NTOHS(*(unsigned int *)(&skb_transport_header(skb)[4]));
	...
	if (vsgInfo) {
		...
		if ((!flow->bIPsecOut) &&
			...
			if ((flow->odev) && !(flow->odev->flags & IFF_UP)) {
				L2blobRefresh = ASF_L2BLOB_REFRESH_DROP_PKT;
				goto gen_indications;
			}
...
/* Only timer based L2 blob refresh  is supported in current release */
	...
	if (flow->bNat) {
		...
		*((unsigned short *)(ptrhdrOffset) + 1) =
			flow->ulNATPorts & 0x0000ffff;
	...
	do {
		...
		if (flow->bPPPoE) {
			/* PPPoE packet.. Set Payload length in PPPoE header */
			*((short *)&(skb->data[(flow->l2blob_len - tunnel_hdr_len)-4])) = ASF_HTONS(skb->len + tunnel_hdr_len + 2);
		}
