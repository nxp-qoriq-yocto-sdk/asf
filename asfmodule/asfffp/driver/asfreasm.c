				       ...
				       int *pIhl, unsigned int ulVSGId)
{
	...
	*pIhl = ((uintptr_t)(fhdr + 1) - (uintptr_t)ipv6h);
	...
	if (((fhdr->frag_off & (ASF_HTONS(IP6_MF))))) {
	...
	if ((likely(ASF_NTOHS(iph->tot_len) > ulMTU)) ||
		...
		if (((skb_pagelen(skb) <= ulMTU)
			...
			if (!bNewSkb) {
				...
				iph->tot_len = ASF_HTONS(skb_pagelen(skb));
		...
		if (bNewSkb) {
			...
			*pOutSkb = skb;
			...
			ip_hdr(skb2)->tot_len = ASF_HTONS(first_frag_len);
			   ...
			   struct sk_buff **pOutSkb)
{
	...
	unsigned int ident = ASF_HTONL(asfReasmGetNextId());
